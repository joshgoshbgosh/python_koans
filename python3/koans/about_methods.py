...
class AboutMethods(Koan):
    ...
    def test_calling_methods_in_same_class_with_explicit_receiver(self):
        def function_with_the_same_name(a, b):
            return a * b
                                                                        #had to look for this one.  i see how it was added
        self.assertEqual(7, self.function_with_the_same_name(3,4))    #instead of muliplied like the above one.  But not sure why
                                                                    #i looked up explicit receiver for python doc. but didn't quite
    # ------------------------------------------------------------------ #see why it did that.
    ...
    def test_attributes_with_double_underscore_prefixes_are_subject_to_name_mangling(self):
        rover = self.Dog()
        with self.assertRaises(AttributeError): password = rover.__password() #most of what i saw from looking up the assert raises was
                                                                             #test errors.  I did not get this one but saw the answer
        # But this still is!                                                #I guess that is why its an Attribute error
